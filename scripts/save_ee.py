...
parser = ap.ArgumentParser(description='select moller events and save')
...
for run, file_ in files.items():
    if exists(join(path, 'data_{}_ee.npz'.format(run))):
        continue
    ...
    data.save(join(path, 'data_{}_ee.npz'.format(run)))
    del data
